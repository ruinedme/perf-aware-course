...
3. Parser optimizations:
  1. avoid per character function-call overhead
  2. user pointer walks and tight loops
  3. prefer zero-copy for strings when possible. (emit view into buffer with offsets rather than copying)
  4. move the number parser into the JSON parser and have the parser return doubles by default. This would allow the parser to avoid some memcpy and other allocations which could make this faster.
