...
static void NewTestWave(repetition_tester *Tester, u64 TargetProcessedByteCount, u64 CPUTimerFreq, u32 SecondsToTry)
{
...
static bool IsTesting(repetition_tester *Tester)
{
    if (Tester->Mode == TestMode_Testing)
    {
        ...
        {
            if (Tester->OpenBlockCount != Tester->ClosedBlockCount)
            {
            ...
            if (Tester->Mode == TestMode_Testing)
            {
                ...
                if (Results->MinTime > ElapsedTime)
                {
                    ...
                    {
                        PrintTime("Min", (f64)Results->MinTime, Tester->CPUTimerFreq, Tester->BytesAccumulatedOnThisTest);
                        printf("               \r");
                    }
                }

                Tester->OpenBlockCount = 0;
                Tester->ClosedBlockCount = 0;
